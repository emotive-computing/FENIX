...
% remove ventricles and WM signal (vw) using canonical masks 
    ...
    % to consider: using unsmoothed data
...
imgs = filenames(fullfile(datadir, 'sub-*', 'func', 'swdcr*02_bold*.nii'), 'char'); 
...
for i=2:subjn
    ...
    % main part
    dat_denoised = canlab_connectivity_preproc(dat, 'vw', 'windsorize', 5, 'hpf', [0.005], TR, 'no_plots');

    fname = fullfile(outdir, [subj '_swdcr_cleaned_hpf.mat']);
    save(fname, 'dat_denoised', '-v7.3');
end
