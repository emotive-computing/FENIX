...
for i = 1:length(DSGN.subjects) 
    ...
    if ~isempty(nii) % if has nii
        ...
        %% create IV design
        heat1_down = struct('name',{{'heat1_down'}}, ...
                    'onset', {{[]}}, ...
                    'duration', {{[]}},...
                    'tmod',{{0}});
        heat1_neut = struct('name',{{'heat1_neut'}}, ...
                    'onset', {{[]}}, ...
                    'duration', {{[]}},...
                    'tmod',{{0}});
        heat1_up = struct('name',{{'heat1_up'}}, ...
                    'onset', {{[]}}, ...
                    'duration', {{[]}},...
                    'tmod',{{0}});
        heat2_down = struct('name',{{'heat2_down'}}, ...
                    'onset', {{[]}}, ...
                    'duration', {{[]}},...
                    'tmod',{{0}});
        heat2_neut = struct('name',{{'heat2_neut'}}, ...
                    'onset', {{[]}}, ...
                    'duration', {{[]}},...
                    'tmod',{{0}});
        ...
        stim_dat_fname = [sid, '_ptt_fnirs.mat'];
        try % import stim data saved by matlab during fnirs 
            stim_dat = importdata(['/Users/marta/Documents/DATA/FENIX/Behavior_FNIRS/timings_ptt_fnirs/',stim_dat_fname]);
        catch
        ...
        if save_files
            fnirs_file_dir = [sid, '_fnirs_modeling/'];
            mkdir(nii(1).folder,fnirs_file_dir)
            mdl_path = [nii(1).folder,'/',fnirs_file_dir];
