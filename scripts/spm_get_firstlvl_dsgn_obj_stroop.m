...
function DSGN = get_firstlvl_dsgn_obj_stroop()
    
    % INPUT DATA ---------------------------------------------------------- %
    DSGN.metadata = "FENIX stroop: GLM first level analysis";
    DSGN.modeldir = '/Users/marta/Documents/DATA/FENIX/Imaging/firstlvl/stroop'; % dir to store firslvl results
    
    DSGN.funcnames = {'func/sw*stroop*.nii'}; % smoothed preproc files
    DSGN.subjects = {};
    fnames = dir('/Users/marta/Documents/DATA/FENIX/Imaging/preprocessed/sub*'); % get subject names w/ subject data 
    
    for i = 1:length(fnames)
        this_f = fnames(i);
        runs = dir([this_f.folder, '/', this_f.name, '/func/sw*stroop*.nii']);
    ...
    % MODELING ------------------------------------------------------------ %
    DSGN.conditions = {{'stroop_con', 'stroop_incon'}}; % other 
    
    DSGN.contrasts = {{{'stroop_con'}, {'stroop_incon'}},...
                      {{'stroop_con'}, {'stroop_incon'}},...
                      {{'stroop_con'}, {'stroop_incon'}}};
                  
    DSGN.contrastweights = {[1 0], [0 1], [1 -1]};
    
    DSGN.contrastnames = {'congruent','incongruent','con_vs_incon'}; 
    
    % will specify contrasts in 2nd level

    DSGN.multireg = '../../nuisance_mat/nuisance_run3.mat'; % things regressed out during preproc
